...
class plotter():
    ...
    def plot_ping(self,data):       
        ...
        length = samples*channels
        self.x_axis_length2 = (samples/4.0)*(1.0/sample_rate)
        ...
        #self.x = np.arange(starting_sample*(time/samples),distance, time/samples)
        #print self.x
        self.a2 = values[starting_sample + 0:int(length):channels]
        #print self.a
        self.b2 = values[starting_sample + 1:int(length):channels]
        self.c2 = values[starting_sample + 2:int(length):channels]
        self.d2 = values[starting_sample + 3:int(length):channels]

        #print len(self.a1)
        #print range(len(self.a1)-1)

        self.a3 = [0]*int(samples/4)
        self.b3 = [0]*int(samples/4)
        self.c3 = [0]*int(samples/4)
        self.d3 = [0]*int(samples/4)
        #print samples/4
        ...
        self.N2 = len(self.a2)   #number of samples in channel1
        Fs = sample_rate
        Ts = 1.0/Fs    
        #x = np.linspace(0.0, N*Ts, N)    
        self.yf2 = scipy.fftpack.fft(self.a2)
        index, value = max(enumerate(abs(self.yf2)), key=operator.itemgetter(1))

        self.xf2 = np.linspace(0.0, 1.0/(2.0*Ts), self.N2/2)

        legends = [None]*channels  #set up ledgends for x channels
        wave = [None]*len(self.a2)  #make empty list

        n = len(self.a2)   #length of samples (samplecount/4 from AdvanTech driver c++)

        self.t2 = np.arange(0,n*Ts,Ts)  #resolution of sampling, ie 1 MS/s = 1*10^-6

        if len(self.t2) > n:
            self.t2 = self.t2[:-1]  #make sure len(t) is = to len(n), shave the last number off            
    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            rospy.Subscriber('/hydrophones/pingraw', Pingdata, self.ping_raw)
            ...
            if len(self.t) == len(self.a):
                ...
                self.ax[1].plot(xvalues2,cvalues2, linewidth=2.0, label='Hydrophone C')
                self.ax[1].plot(xvalues2,dvalues2, linewidth=2.0, label='Hydrophone D')

                self.ax[1].legend(loc="upper left", fontsize=10)
                self.ax[1].set_title("Conditioned Signals")#, weight = 'bold', size = 12, x = 0.5, y = 1.02, horizontalalignment='center')
                #self.ax[1].set_xlabel('Time (seconds)', size = 25, weight = 'bold', x = 0.5, y = 0)
                self.ax[1].set_ylabel('Amplitude')#, size = 25, weight = 'bold', x = 0, y = 0.5)
                #self.ax[0].set_ylim(-5,5)
                self.ax[1].set_xlim(0,self.x_axis_length2)
                ...
                if heartbeat >= 150:
                    rospy.signal_shutdown("heartbeat")

            rate.sleep()

        plt.close('all')
    
    #rospy.on_shutdown(heartbeat)
